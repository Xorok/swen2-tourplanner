CREATE TABLE tt_tour_type
(
    tt_id VARCHAR(255) PRIMARY KEY
);

CREATE TABLE td_tour_difficulty
(
    td_id VARCHAR(255) PRIMARY KEY
);
...
(
    ...
    t_tt_type    VARCHAR(255) NOT NULL REFERENCES tt_tour_type (tt_id),
...
(
    ...
    tl_td_difficulty VARCHAR(255)     NOT NULL REFERENCES td_tour_difficulty (td_id),
    tl_rating        INTEGER          NOT NULL check (tl_rating > 0 AND tl_rating <= 100)
);

-- Add default tour types
...
-- Add default tour difficulties
INSERT INTO td_tour_difficulty(td_id)
VALUES ('EASY'),
       ('MEDIUM'),
       ('HARD');
